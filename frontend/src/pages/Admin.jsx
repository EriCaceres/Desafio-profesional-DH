...
export default function Admin() {
  ...
  const handleSaveFeature = async (e) => {
    ...
    try {
      ...
      setFeatureError(e.response?.data?.message || "Error al guardar característica");
    } finally {
  ...
  const handleSaveCategory = async (e) => {
    ...
    try {
      ...
      setCatError(e.response?.data?.message || "Error al guardar categoría");
    } finally {
      ...
      <div className="max-w-5xl mx-auto px-4 space-y-6">
        ...
        <section className="bg-white p-4 rounded shadow space-y-3">
          ...
          <form onSubmit={handleSubmit} className="space-y-3">
            ...
            <div>
              ...
              <textarea className="border rounded px-2 py-1 w-full text-sm" rows={3} value={description} onChange={(e) => setDescription(e.target.value)} />
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              <div>
                <label className="block text-sm font-medium mb-1">Duración (min)</label>
                <input type="number" className="border rounded px-2 py-1 w-full" value={durationMin} onChange={(e) => setDurationMin(e.target.value)} />
              </div>
              <div>
                <label className="block text-sm font-medium mb-1">Precio desde</label>
                <input type="number" className="border rounded px-2 py-1 w-full" value={priceFrom} onChange={(e) => setPriceFrom(e.target.value)} />
              </div>
              <div>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-2 text-sm">
                {features.length === 0 && <span className="text-gray-500">Primero creá características en la sección de abajo.</span>}
                {features.map((f) => (
        ...
        <section className="bg-white p-4 rounded shadow space-y-3">
          ...
          <form onSubmit={handleSaveCategory} className="space-y-3">
            <div>
              <label className="block text-sm font-medium mb-1">Nombre *</label>
              <input className="border rounded px-2 py-1 w-full" value={catName} onChange={(e) => setCatName(e.target.value)} />
            </div>
            <div>
